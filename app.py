...
# Override các cấu hình lựa chọn để đồng bộ với yêu cầu hiện tại.
APP_TITLE = "Nhật ký hỗ trợ khách hàng"
CHANNEL_OPTIONS = [
    "Group ĐTP-AM-Hỗ trợ",
    "Điện thoại",
...
def ensure_support_records_schema(db: sqlite3.Connection) -> None:
    columns = {
        row["name"]
        for row in db.execute("PRAGMA table_info(support_records)").fetchall()
    }
...
def ensure_default_services() -> None:
    ...
    existing_names = {
        row["name"]
        for row in db.execute("SELECT name FROM services").fetchall()
    }
    missing_services = [
...
def seed_sample_data() -> None:
    ...
    entry_count = db.execute("SELECT COUNT(*) AS total FROM support_records").fetchone()["total"]
    if entry_count > 0:
        return

    service_lookup = {
        row["name"]: row["id"]
        for row in db.execute("SELECT id, name FROM services").fetchall()
    }
...
def serialize_service(row: sqlite3.Row) -> dict[str, Any]:
    return {
        "id": row["id"],
        "name": row["name"],
...
def fetch_services(include_inactive: bool = True) -> list[dict[str, Any]]:
    ...
    """
    return [serialize_service(row) for row in db.execute(sql, params).fetchall()]


def fetch_service(service_id: int) -> sqlite3.Row | None:
    db = get_db()
    return db.execute(
        """
...
def build_dashboard(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    channel_counter = Counter(entry["channel"] for entry in entries)
    unresolved = status_counter["Đang xử lý"] + status_counter["Chờ phản hồi"]
...
def build_service_summary(entries: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for service_name, service_entries in sorted(
        ...
        channel_counter = Counter(entry["channel"] for entry in service_entries)
        customer_counter = Counter(
            entry["customer_name"]
        ...
        rows.append(
            {
                ...
                "unique_customers": len(customer_counter),
                "done_count": status_counter.get("Đã xử lý", 0),
                "in_progress_count": status_counter.get("Đang xử lý", 0),
                "waiting_count": status_counter.get("Chờ phản hồi", 0),
                "ultra_count": channel_counter.get("Ultra khách", 0),
                "phone_count": channel_counter.get("Điện thoại", 0),
                "zalo_count": channel_counter.get("Zalo", 0),
                "onsite_count": channel_counter.get("Trực tiếp", 0),
                "other_count": channel_counter.get("Khác", 0),
...
def create_service():
    ...
    row = fetch_service(cursor.lastrowid)
...
def create_entry():
    ...
    db.commit()
    created = fetch_entry(cursor.lastrowid)
...
def delete_entry(entry_id: int):
    ...
    db = get_db()
    db.execute("DELETE FROM support_records WHERE id = ?", (entry_id,))
    db.commit()
